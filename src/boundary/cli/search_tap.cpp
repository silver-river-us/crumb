...
std::string render_text(std::string_view query, const domain::DirectoryPath& directory,
    ...
    for (const auto& span : result.trace) {
        const auto offset_size = total == 0
                                      ? std::size_t{}
                                      : std::min(bar_width, static_cast<std::size_t>(
                                                               span.offset_us * bar_width / total));
        const auto available_width = std::max<std::size_t>(1, bar_width - offset_size);
        const auto bar_size = total == 0
                                  ? std::size_t{1}
        ...
        output << "  " << std::left << std::setw(static_cast<int>(name_width)) << span.name
               ...
               << std::string(bar_width - offset_size - bar_size, ' ')
               << "| " << std::right << std::setw(9) << format_duration(span.duration_us)
               << " @ " << std::setw(9) << format_duration(span.offset_us);
...
std::string render_html(std::string_view query, const domain::DirectoryPath& directory,
    ...
    output << "<!doctype html>\n<html lang=\"en\"><head><meta charset=\"utf-8\">"
              ...
              ".track{height:20px;background:#252b36;border-radius:4px;overflow:hidden;position:relative}"
              ...
              "border-bottom:1px solid #303744;text-align:left}code{color:#b9d4ff}</style></head><body><main>";
    output << "<h1>Crumb search tap</h1><div class=\"meta\">query <code>"
           << html_escape(query) << "</code> · directory <code>" << html_escape(directory.value())
           << "</code> · total " << format_duration(total) << " · " << result.matches.size()
           << " matches</div>";
    ...
    for (const auto& span : result.trace) {
        ...
        output << "<div class=\"row\"><div class=\"name\">" << html_escape(span.name)
               ...
               << std::fixed << std::setprecision(2) << width << "%;left:" << offset
               << "%\"></div></div><div>"
