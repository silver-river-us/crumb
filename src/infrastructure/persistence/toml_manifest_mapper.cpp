...
std::optional<std::string> string_field(const std::unordered_map<std::string, std::string>& fields,
                                        std::string_view key) {
    if (const auto it = fields.find(std::string(key)); it != fields.end())
        return unquote(it->second);
    return std::nullopt;
}
std::optional<std::uintmax_t> integer_field(
    const std::unordered_map<std::string, std::string>& fields, std::string_view key) {
    if (const auto value = string_field(fields, key)) {
        std::uintmax_t n{};
        auto [p, e] = std::from_chars(value->data(), value->data() + value->size(), n);
        if (e == std::errc{}) return n;
    }
    return std::nullopt;
}
}  // namespace
...
std::expected<domain::DirectoryManifest, std::string> TomlManifestMapper::fromToml(
    ...
    auto flush = [&] {
        if (!section.empty() && section.starts_with("files."))
            records[unquote(section.substr(6))] = fields;
        fields.clear();
    };
        ...
        try {
            ...
            for (const auto& [key, value] : values) {
                if (key == "content_hash")
                    ...
                    if (auto n = integer_field(values, key))
                        metadata.created_ns = static_cast<std::int64_t>(*n);
                } else if (key == "inode")
                    metadata.inode = integer_field(values, key);
                else if (key == "device")
                    metadata.device = integer_field(values, key);
                else if (key == "title")
