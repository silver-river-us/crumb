...
std::expected<void, std::string> BinarySearchIndexRepository::save(const domain::DirectoryPath& root, const domain::SearchIndex& index) {
    ...
    try {
        ...
        std::error_code ignored; std::filesystem::remove(temporary, ignored); return std::unexpected(error.what());
    }
    return {};
}
...
std::expected<domain::SearchIndex, std::string> BinarySearchIndexRepository::load(const domain::DirectoryPath& root) const {
    ...
    std::vector<Bytef> compressed(compressed_size);
    if (!in.read(reinterpret_cast<char*>(compressed.data()), static_cast<std::streamsize>(compressed_size))) return std::unexpected("truncated search index");
