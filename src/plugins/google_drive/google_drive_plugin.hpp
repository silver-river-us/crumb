...
public:
    ...
    load(const domain::DirectoryPath&) override;
...
private:
    ...
    [[nodiscard]] static domain::DirectoryManifest with_path(
        const domain::DirectoryManifest&, domain::DirectoryPath);
...
public:
    explicit DriveSearchIndexRepository(ports::SearchIndexRepository& delegate) : delegate_(delegate) {}

    void set_cache_root(std::filesystem::path cache_root) { cache_root_ = std::move(cache_root); }
    std::expected<void, std::string> save(
        const domain::DirectoryPath&, const domain::SearchIndex&) override;
...
public:
    ...
    list_regular_files(const domain::DirectoryPath&) override;
...
public:
    ...
    std::expected<domain::FileMetadata, std::string> extract(
        const domain::DirectoryPath&, const domain::FileName&, domain::FileMetadata) override;
...
public:
    GoogleDrivePlugin(infrastructure::NativeFileSystem& native,
                      ports::ManifestRepository& manifests,
                      ports::SearchIndexRepository& index,
                      ports::FingerprintService& fingerprints,
                      ports::IdGenerator& ids,
                      ports::Clock& clock)
        : filesystem_(native), extractor_(),
          manifests_(manifests), index_(index),
          reconcile_(manifests_, filesystem_, fingerprints, extractor_, ids, clock),
          rebuild_index_(manifests_, filesystem_, index_), search_(manifests_, filesystem_, &index_) {}
