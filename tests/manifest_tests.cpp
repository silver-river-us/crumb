...
int main() {
    ...
    assert(parsed->find(FileName::create("annual.report.pdf"))->id.value() == "01K1AC4K3N7JZM5F21V6PH8QRT");
    assert(parsed->find(FileName::create("annual.report.pdf"))->metadata.external_url.value() == "https://drive.google.com/open?id=abc_123-xyz");
    parsed->rename(FileName::create("annual.report.pdf"), FileName::create("renamed [final].pdf"));
    assert(parsed->find(FileName::create("renamed [final].pdf"))->id.value() == "01K1AC4K3N7JZM5F21V6PH8QRT");
    auto unknown = text;
    ...
    auto unsupported = mapper.fromToml("version = 2\ndirectory_id = \"01K1AB5YZ4QH7M2D8E3F9G6JNX\"\ngenerated_at = \"now\"\n");
